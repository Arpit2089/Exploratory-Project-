...
#   - DataLoaders are created ONCE per run and passed through the call chain.
#     Previously _sequential_train_gpu re-created loaders on every generation
#     call, killing persistent-worker performance and wasting startup time.
#   - USE_PRETRAINED_BACKBONE: gen-0 seeds load ResNet18 ImageNet weights.
#   - USE_MULTI_GPU: wraps each model with nn.DataParallel when available.
# =============================================================================
...
def _maybe_load_pretrained(child_model, child_graph, child_sd_path, cfg):
    """
    Load pretrained ResNet18 ImageNet weights into child_model when:
      1. cfg.USE_PRETRAINED_BACKBONE is True, AND
      2. child_sd_path is None (no Lamarckian inherited weights — gen-0 only)
    """
    if not getattr(cfg, 'USE_PRETRAINED_BACKBONE', False):
        return
    if child_sd_path is not None:
        return  # has inherited weights → skip pretrained loading

    node_map = getattr(child_graph, '_pretrained_node_map', None)
    if node_map is None:
        logger.warning(
            "USE_PRETRAINED_BACKBONE=True but graph has no _pretrained_node_map "
            "— skipping pretrained weight loading for this seed."
        )
        return
    ...
    try:
        ...
        load_pretrained_resnet18_weights(child_model, node_map, num_classes)
    except Exception as e:
        logger.warning("Pretrained weight loading failed: %s — using random init", e)


def _wrap_for_multi_gpu(model, cfg):
    """
    Wrap model with DataParallel if multiple GPUs available and
    cfg.USE_MULTI_GPU is True.  Returns (wrapped_model, is_wrapped).
    """
    import torch
    import torch.nn as nn

    use_multi = getattr(cfg, 'USE_MULTI_GPU', False)
    if not use_multi:
        return model, False
    ...
    logger.debug("DataParallel: using %d GPUs", num_gpus)
    return wrapped, True
...
def _train_one_child_gpu(pc, cfg, train_epochs, train_lr,
    ...
    Receives pre-built loaders — does NOT create new ones.
    """
    import torch
    ...
    try:
        ...
        # ── 2a. Pretrained weights (gen-0 seeds only) ─────────────────────
        _maybe_load_pretrained(child_model, child_graph, child_sd_path, cfg)

        # ── 2b. Lamarckian inherited weights (gen-1+ children) ────────────
        ...
        # ── 3. Build teacher for ANM distillation ─────────────────────────
        teacher_model = None
        ...
        # ── 5b. Standard training (with optional DataParallel) ────────────
...
def _sequential_train_gpu(payloads, cfg, train_epochs, train_lr,
    ...
    Receives pre-built data loaders — does NOT recreate them.

    Key fix: loaders are now passed in from run_lemonade, so they are
    created exactly ONCE per full NAS run instead of once per generation.
    This preserves persistent DataLoader workers across all generations.
    """
    trained = []
    n       = len(payloads)
...
def _train_all(payloads, cfg, train_epochs, train_lr, desc, device,
    ...
    train_loader / val_loader are passed through to the GPU path so they
    are reused across generations without being recreated.
    """
...
def run_lemonade(init_graphs, cfg, train_loader, val_loader, device, run_dir):
    ...
    Parameters
    ----------
    init_graphs  : list[ArchitectureGraph] OR list[Individual]
    cfg          : NASConfig
    train_loader : DataLoader — created ONCE in main.py and reused here
    val_loader   : DataLoader — created ONCE in main.py and reused here
    ...
    logger.info(
        ...
        "device=%s amp=%s multi_gpu=%s pretrained=%s input_size=%s",
        ...
        getattr(cfg, 'USE_MULTI_GPU', False),
        getattr(cfg, 'USE_PRETRAINED_BACKBONE', False),
        input_size,
    )
    ...
    if init_graphs and isinstance(init_graphs[0], Individual):
        ...
        logger.info("Received %d pre-built Individuals (pretrained backbone path)",
                    len(population))
    else:
    ...
    # Free pretrained model memory now that weights are serialised
    for ind in population:
        if ind.model is not None:
            ind.model.cpu()
            ind.model = None
    ...
    if payloads_g0:
        trained_seeds = _train_all(
            ...
            device=device,
            train_loader=train_loader,   # passed through — not recreated
    ...
    for gen in range(1, cfg.GENERATIONS + 1):
        ...
        try:
            ...
            # ── STEP 3: Generate npc candidate children ──────────────────
            candidates = []
            for p in parents:
                try:
            ...
            for child, parent, approx in candidates:
                try:
                    ...
                    error_logger.error("Cheap eval failed for %s: %s", child.id, e)
            ...
            if payloads:
                trained_children = _train_all(
                    ...
                    device=device,
                    train_loader=train_loader,   # reused — not recreated
                    val_loader=val_loader,
                )
